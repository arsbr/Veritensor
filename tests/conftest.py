...
try:
    ...
    MALWARE_GEN_AVAILABLE = True
except ImportError:
    MALWARE_GEN_AVAILABLE = False
...
def setup_test_env():
    ...
    if MALWARE_GEN_AVAILABLE:
        gen = MalwareGenerator(TEST_DIR)
        gen.generate_all()
    else:
        # Fallback: we create empty files so that the tests don't crash due to missing paths.
        (TEST_DIR / "rce_simple.pkl").write_bytes(b"fake_malware")
        (TEST_DIR / "model_infected.pt").write_bytes(b"fake_malware")
...
def reset_config_singleton():
    """Ensures every test starts with a fresh config."""
    ConfigLoader.reset()
