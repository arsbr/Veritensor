...
@patch("veritensor.engines.content.injection.PIIScanner.scan", return_value=[])
def test_rag_clean_files(mock_pii, tmp_path):
...
@patch("veritensor.engines.content.injection.PIIScanner.scan", return_value=[])
def test_scan_text_in_memory(mock_pii):
